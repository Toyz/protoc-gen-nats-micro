...
func main() {
	...
	protogen.Options{}.Run(func(gen *protogen.Plugin) error {
		...
		for _, f := range gen.Files {
			...
			// For Go: Extract directory from GeneratedFilenamePrefix (derived from go_package)
			// For non-Go: Extract directory from proto source path (e.g., "auth/v1/auth.proto" -> "auth/v1")
			...
			// For non-Go: use the directory path (e.g., "gen/order/v1")
			...
			if len(f.Services) > 0 && !generatedShared[pkgKey] {
				...
				} else if webTsLang, ok := lang.(*generator.WebTSLanguage); ok {
					if err := webTsLang.GenerateShared(sharedFile, f); err != nil {
						return fmt.Errorf("generate shared: %w", err)
					}
				}
			}
